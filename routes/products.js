...
const { _Collection } = require("../models/_collections");
...
router.get("/collection/:id", async (req, res) => {
    try {
        ...
        const products = await Product.find({ _collection: collection.title });
...
router.post("/", async (req, res) => {
    ...
    if (duplicate)
        return res
            .status(400)
            .send({ value: "Similar product already exists" });
    ...
    try {
        const correspondingCollection = await _Collection.findById(
            body._collection
        );
        const collectionProducts = correspondingCollection.products;

        const product = new Product({
            ...body,
            _collection: correspondingCollection.title,
        });
        await product.save();
        collectionProducts.push(product._id);
        await correspondingCollection.updateOne({
...
router.put("/:id", async (req, res) => {
    ...
    try {
        ...
        let correspondingCollection = null;

        if (body._collection) {
            correspondingCollection = await _Collection.findById(
                body._collection
            );
            body._collection = correspondingCollection.title;
        }
